    ...
    <div className="flex flex-col min-h-screen bg-aed-base">
      <header className="h-[52px] bg-white border-b border-aed-border flex items-center px-5 gap-3 flex-shrink-0 sticky top-0 z-10">
        ...
        <div className="flex-1 min-w-0 flex items-center gap-2">
          <h1 className="text-[18px] font-bold text-slate-800 truncate">
        ...
        <span className="text-[13px] text-slate-500">{pagination?.total ?? 0} unidades</span>
      </header>
      ...
      <div className="flex-1 p-5 flex flex-col gap-4">
        ...
        {loading && !result ? (
          <div className="flex items-center justify-center py-20 text-slate-500 text-[15px] gap-2">
          ...
          <div className="flex flex-col items-center justify-center py-20 text-slate-500">
            ...
            <p className="text-[15px] font-medium">Sin unidades encontradas</p>
                  ...
                  >
                    <div className="flex items-start justify-between mb-2.5">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-lg bg-brand-soft border border-brand-soft flex items-center justify-center flex-shrink-0">
                        ...
                        <div>
                          <h3 className="font-bold text-slate-800 text-[15px] group-hover:text-brand transition-colors">
                            {item.nomenclatura}
                          </h3>
                          {(item.tipo || item.inventario) && (
                            <p className="text-[13px] text-slate-500 mt-0.5 truncate max-w-[160px]">
                              {item.tipo || item.inventario}
                            </p>
                          )}
                        </div>
                      </div>
                      <svg className="w-4 h-4 text-slate-200 group-hover:text-brand transition-colors flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    {item.compradorPrincipal && (
                      <div className="flex items-center gap-1.5 mb-2">
                        ...
                        <span className="text-[14px] text-slate-600 truncate">{item.compradorPrincipal}</span>
                        {item.nroId && (
                          <span className="text-[12px] text-slate-500 font-mono ml-1 flex-shrink-0">{item.nroId}</span>
                        )}
                      </div>
                    )}
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-1.5 min-w-0">
                        {item.estado && (
                          <span className={`text-[12px] font-bold px-1.5 py-0.5 rounded-full flex-shrink-0 ${estadoColor(item.estado)}`}>
                            {item.estado}
                          </span>
                        )}
                        {saldo && (
                          <span className={`text-[12px] font-semibold tabular-nums flex-shrink-0 ${saldoNum > 0 ? 'text-emerald-600' : 'text-slate-500'}`}>
                            {saldo}
                          </span>
                        )}
                      </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="btn-secondary text-[14px] py-1.5 px-3 disabled:opacity-40"
                  >Anterior</button>
                  <button
