...
export default function ApartamentoDetalle() {
    ...
    return (
      <div className="flex-1 flex items-center justify-center min-h-screen">
        <div className="text-center">
          <p className="text-red-500 text-[13px] mb-4">{error || 'No encontrado'}</p>
          <button onClick={() => navigate(-1)} className="btn-primary text-[13px]">Volver</button>
        </div>
      </div>
    );
  }
  ...
  const movFields = movimientos.length > 0 ? Object.keys(movimientos[0].datos || {}) : [];
    ...
    <div className="flex flex-col min-h-screen bg-aed-base">
      ...
      <header className="h-[52px] bg-white border-b border-aed-border flex items-center px-5 gap-3 flex-shrink-0 sticky top-0 z-10">
        ...
        <span className="text-[11px] text-slate-400">
          {encargo?.nombre}
          {encargo?.codigo && <span className="ml-1 text-blue-400">({encargo.codigo})</span>}
      ...
      <div className="flex flex-col gap-3 p-5">
        ...
        <Accordion icon={BarChart3} title="Estructura financiera y saldos" badge={finEntries.length} defaultOpen>
          {finEntries.length > 0
            ...
            : <p className="px-4 py-4 text-[12px] text-slate-400 italic">Sin datos financieros</p>}
        </Accordion>
