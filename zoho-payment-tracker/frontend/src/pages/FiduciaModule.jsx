    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/30 backdrop-blur-[1px]">
      <div className="bg-white rounded-2xl border border-aed-border shadow-[var(--shadow-overlay)] w-full max-w-md mx-4 p-6">
        <div className="flex items-center justify-between mb-5">
          ...
          <button
            onClick={onClose}
            className="w-7 h-7 flex items-center justify-center rounded-lg border border-aed-border hover:bg-aed-base text-slate-500"
          >
            ×
          </button>
        </div>
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-colors ${
            dragging ? 'border-brand bg-brand-soft' : 'border-aed-border hover:border-brand hover:bg-aed-base'
          }`}
        >
          <input
            ref={inputRef}
          ...
          />
          {uploading ? (
            <div className="flex items-center justify-center gap-2 text-brand text-[15px]">
            ...
            <>
              ...
              <p className="text-[15px] font-medium text-slate-600">Arrastra los Excel aquí o haz clic</p>
              <p className="text-[13px] text-slate-500 mt-1">Archivos .xlsx o .xls — puedes subir varios</p>
            </>
          )}
        </div>
        ...
        {results && (
          <div className="mt-3 flex flex-col gap-1.5">
            {results.map((r, i) => (
              <div
                key={i}
                className={`text-[14px] px-3 py-2 rounded-lg flex items-center gap-2 ${
  ...
  return (
    <div className="flex flex-col min-h-screen bg-aed-base">
      ...
      <header className="h-[52px] bg-white border-b border-aed-border flex items-center px-5 gap-3 flex-shrink-0 sticky top-0 z-10">
        ...
        <span className="text-[14px] text-slate-500">Encargos</span>
        <div className="ml-auto">
          <button onClick={() => setShowUpload(true)} className="btn-primary text-[14px] px-3 py-1.5 flex items-center gap-1.5">
      ...
      <div className="flex-1 p-5 flex flex-col gap-4">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-slate-500">
            ...
            <p className="text-[15px] font-medium">Sin encargos importados</p>
            <p className="text-[14px] mt-1">Usa "Importar Excel" arriba para comenzar</p>
          </div>
        ) : (
              ...
              <tbody>
                {data.data.map((enc) => (
                  ...
                  >
                      ...
                      {editingId === enc.id ? (
                        <div className="flex flex-col gap-1" onClick={(e) => e.stopPropagation()}>
                          <input
                            ...
                            className="input text-[14px] px-2 py-1"
                          ...
                          <input
                            ...
                            className="input text-[14px] px-2 py-1 font-mono w-28"
                            placeholder="Código"
                          />
                          <div className="flex gap-1.5 mt-1">
                            <button onClick={(e) => handleSaveEdit(e, enc.id)} className="btn-primary text-[13px] px-2 py-1">Guardar</button>
                            <button onClick={handleCancelEdit} className="btn-secondary text-[13px] px-2 py-1">Cancelar</button>
                          </div>
                        </div>
                      ) : (
                        <>
                    ...
                    <td className="px-4 py-3 text-slate-500 text-[14px] truncate max-w-xs">{enc.archivoNombre}</td>
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {enc.hojas.map((h) => (
                          <span key={h.id} className="text-[12px] bg-aed-base border border-aed-border text-slate-500 px-2 py-0.5 rounded-full">
                            {h.nombreHoja} ({h.totalFilas})
                          </span>
                        ))}
                      </div>
                    </td>
                    <td className="px-4 py-3 text-slate-500 text-[13px] whitespace-nowrap">{formatDateTime(enc.createdAt)}</td>
                    <td className="px-4 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                      {editingId !== enc.id && (
                        <button
                          onClick={(e) => startEdit(e, enc)}
                          className="text-slate-500 hover:text-brand transition-colors p-1 rounded mr-1"
                          title="Editar"
                        >
              ...
              <div className="px-4 py-3 border-t border-aed-border flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="btn-secondary text-[13px] py-1 px-2 disabled:opacity-40"
                  >Anterior</button>
                  <button
